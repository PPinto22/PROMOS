...
class FitnessOptions:
    def __init__(self, config):
        self.complexity_type = ComplexityType(config['complexity'])
...
class FitnessAdjuster:
    ...
    def _step(self, evaluation):
        complexity = self.get_complexity(evaluation)

        if complexity < self.options.lower_limit:
            return evaluation.fitness  # Do not penalize

        distance = complexity - self.options.lower_limit
    ...
    def get_pop_adjusted_fitness(self, pop_evaluations):
        f = self.options.penalty_function
        pop_max = max(self.get_complexity(e) for e in pop_evaluations) if f is PenaltyFunction.POPMAX else None
...
class MutationRateController:
    ...
    def adjust(self, complexity):
        ...
        if self.phase is MutationRateController.Phase.DEFAULT:
            if complexity < self.options.upper_limit:
                pass
            else:
            ...
            if complexity > self.options.upper_limit:
                self.simplify()
            elif complexity > self.options.lower_limit:
                pass
            else:
                self.begin_default_phase()
        else:
