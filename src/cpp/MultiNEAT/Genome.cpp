...
namespace NEAT {
    ...
    void Genome::BuildHyperNEATPhenotype(NeuralNetwork &net, Substrate &subst) {
        ...
        bool t_has_loops = HasLoops();
        // dp = depth. Used to activate the network if it has loops. 8 is an arbitrary guess
        int dp = 8;
        ...
        if (subst.m_leaky) {
            for (unsigned int i = net.NumInputs(); i < net.m_neurons.size(); i++) {
                ...
                if (!t_has_loops) {
                    t_temp_phenotype.FeedForward();
                } else {
                    for (int d = 0; d < dp; d++) {
                        t_temp_phenotype.ActivateUseInternalBias();
                    }
                }
    ...
    double Genome::CompatibilityDistance(Genome &a_G, Parameters &a_Parameters) {
        ...
        for (unsigned int i = 0; i < NumNeurons(); i++) {
            ...
            if ((m_NeuronGenes[i].Type() != INPUT) && (m_NeuronGenes[i].Type() != BIAS)) {
                ...
                if (a_G.HasNeuronID(m_NeuronGenes[i].ID())) {
                    ...
                    std::map<std::string, double> neuron_trait_difference = m_NeuronGenes[i].GetTraitDistances(
                            a_G.GetNeuronByID(m_NeuronGenes[i].ID()).m_Traits);
                    // add to the totals
        ...
        t_total_distance =
                ...
                (a_Parameters.ActivationFunctionDiffCoeff *
                 (t_total_num_activation_difference / t_num_matching_neurons));
        ...
        for (auto it = t_total_link_trait_difference.begin(); it != t_total_link_trait_difference.end(); it++) {
            t_total_distance +=
                    (a_Parameters.LinkTraits[it->first].m_ImportanceCoeff * it->second) / t_num_matching_links;
        }
        for (auto it = t_total_neuron_trait_difference.begin(); it != t_total_neuron_trait_difference.end(); it++) {
            t_total_distance +=
                    (a_Parameters.NeuronTraits[it->first].m_ImportanceCoeff * it->second) / t_num_matching_neurons;
        }
    ...
    bool Genome::Mutate_AddNeuron(InnovationDatabase &a_Innovs, const Parameters &a_Parameters, RNG &a_RNG) {
        ...
        if (t_innovid == -1) {
            ...
            } else {   // mate instead of randomizing
    ...
    */
    ...
    void Genome::DivideInitialize(const std::vector<double> &node,
                                  ...
                                  Parameters &params,
                                  const bool &outgoing,
                                  const double &z_coord) {   // Have to check if this actually does something useful here
        //CalculateDepth();
    ...
    void Genome::CollectValues(std::vector<double> &vals, boost::shared_ptr<QuadPoint> &point) {
        ...
        } else {   // Here, Apparently it treats the point a if it is not initialized
